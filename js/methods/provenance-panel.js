/* provenance-panel.js — "Cómo trabajamos": one long series, cell by cell, saying out loud
   ...
   DIVERGENCE FROM THE ANDALUSIA/CAHE COPY (2026-09-05): this viewer ships in Spanish and in
   English, and the shared copy had its chrome hardcoded in Spanish, so an English reader got
   "Ver solo los años observados" under an English section. Every string here is bilingual and
   the language is chosen with ProvenancePanel.setLanguage(). The method vocabulary also grew
   the names this pipeline actually uses (regional_proxy, growth_from_mitchell, …), labelled
   rather than renamed. If this is adopted upstream, Andalucía and CAHE can take this file.
...
let LANG = 'es';
const pick = (o) => (o && (o[LANG] ?? o.es)) ?? o;

// Canonical vocabulary of the data-provenance skill, plus the names this pipeline uses.
// An unknown method still renders, in grey, labelled with its raw name — hiding it would
// defeat the purpose of the panel.
const METHODS = {
    observed:                  { label: { es: 'Observado en la fuente', en: 'Observed in the source' }, color: '#1a4d2e' },
    linear_interpolation:      { label: { es: 'Interpolación lineal entre observaciones', en: 'Linear interpolation between observations' }, color: '#c77b30' },
    interpolated:              { label: { es: 'Interpolación lineal entre observaciones', en: 'Linear interpolation between observations' }, color: '#c77b30' },
    extrapolation:             { label: { es: 'Extrapolación', en: 'Extrapolation' }, color: '#a83232' },
    growth_proxy:              { label: { es: 'Proxy de crecimiento', en: 'Growth proxy' }, color: '#7570b3' },
    distributed_from_aggregate:{ label: { es: 'Reparto desde un agregado', en: 'Distributed from an aggregate' }, color: '#2b7a8c' },
    manual_correction:         { label: { es: 'Corrección manual documentada', en: 'Documented manual correction' }, color: '#8a6d3b' },
    missing:                   { label: { es: 'Sin dato', en: 'No data' }, color: '#9aa0a6' },

    // Names of the agricultural labour pipeline. They are labelled here so a reader can
    // understand the chart, and they are ALSO listed as non-canonical in index.json so they
    // can be reconciled upstream. Naming them on screen and flagging them in the metadata are
    // different jobs; neither is renaming them away.
    original_smoothed:         { label: { es: 'Observado y suavizado', en: 'Observed, then smoothed' }, color: '#3d7350' },
    regional_proxy:            { label: { es: 'Proxy regional: la serie de su región, reescalada', en: 'Regional proxy: its region’s series, rescaled' }, color: '#2b7a8c' },
    regional_average:          { label: { es: 'Media de la región', en: 'Regional average' }, color: '#4a9aa8' },
    growth_from_regional:      { label: { es: 'Tasa de crecimiento de la región', en: 'Growth rate of the region' }, color: '#69adb5' },
    growth_from_mitchell:      { label: { es: 'Tasa de crecimiento de Mitchell (estadísticas históricas)', en: 'Growth rate from Mitchell (historical statistics)' }, color: '#8a5a9b' },
    growth_from_moxlad:        { label: { es: 'Tasa de crecimiento de MOxLAD (América Latina)', en: 'Growth rate from MOxLAD (Latin America)' }, color: '#a8739b' },
    growth_from_etd:           { label: { es: 'Tasa de crecimiento de la Groningen ETD', en: 'Growth rate from the Groningen ETD' }, color: '#7570b3' },
    growth_from_etd_smoothed:  { label: { es: 'Tasa de crecimiento de la Groningen ETD, suavizada', en: 'Growth rate from the Groningen ETD, smoothed' }, color: '#8b87c4' },
    growth_from_owd:           { label: { es: 'Tasa de crecimiento de Our World in Data', en: 'Growth rate from Our World in Data' }, color: '#9b8bd4' },
    gdp_interpolated:          { label: { es: 'Interpolado con el PIB por ancla', en: 'Interpolated against a GDP anchor' }, color: '#c77b30' },
    unspecified:               { label: { es: 'Sin método declarado', en: 'No method declared' }, color: '#9aa0a6' },
};
...
const UI = {
    es: {
        observed: 'Observado', estimated: 'Estimado', nodata: 'Sin dato',
        method: 'Método', source: 'Fuente', confidence: 'Confianza',
        between: 'Interpolado entre', and: 'y',
        empty: 'Esta serie no tiene ningún valor que mostrar.',
        onlyObserved: 'Ver solo los años observados',
        download: 'Descargar esta serie (CSV con procedencia)',
        barCap: (c, p) => `<b>${p} %</b> de los años son dato observado (${c.observadas} de ${c.celdas}); ` +
            `${c.estimadas} estimados${c.sin_dato ? `, ${c.sin_dato} sin dato` : ''}.`,
        ariaSeries: (s) => `${s.label} en ${s.territory}. ${s.cobertura.observadas} de ${s.cobertura.celdas} años observados.`,
        ariaBar: (c) => `${c.observadas} de ${c.celdas} años observados`,
        ariaMatrix: (label) => `Cobertura por territorio y año de ${label}`,
        everyN: (n) => ` · un año de cada ${n}`,
        bc: (y) => `${y} a.C.`,
    },
    en: {
        observed: 'Observed', estimated: 'Estimated', nodata: 'No data',
        method: 'Method', source: 'Source', confidence: 'Confidence',
        between: 'Interpolated between', and: 'and',
        empty: 'This series has no value to show.',
        onlyObserved: 'Show observed years only',
        download: 'Download this series (CSV with provenance)',
        barCap: (c, p) => `<b>${p} %</b> of the years are observed (${c.observadas} of ${c.celdas}); ` +
            `${c.estimadas} estimated${c.sin_dato ? `, ${c.sin_dato} with no data` : ''}.`,
        ariaSeries: (s) => `${s.label} in ${s.territory}. ${s.cobertura.observadas} of ${s.cobertura.celdas} years observed.`,
        ariaBar: (c) => `${c.observadas} of ${c.celdas} years observed`,
        ariaMatrix: (label) => `Coverage by territory and year for ${label}`,
        everyN: (n) => ` · one year in every ${n}`,
        bc: (y) => `${y} BC`,
    },
};
...
const methodInfo = (m) => {
    const hit = METHODS[m];
    return hit ? { label: pick(hit.label), color: hit.color }
               : { label: m || (LANG === 'en' ? 'not specified' : 'sin especificar'), color: '#9aa0a6' };
};
const esc = (s) => String(s ?? '').replace(/[&<>"']/g, (c) =>
    ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]));

const fmtValue = (v, unit) => v == null ? '—'
    : new Intl.NumberFormat(locale(), { maximumFractionDigits: Math.abs(v) < 10 ? 2 : 0 }).format(v)
      + (unit ? ' ' + unit : '');

const fmtYear = (y) => y < 0 ? t().bc(Math.abs(y)) : String(y);
...
function drawSeries(host, serie, opts = {}) {
    ...
    if (!withValue.length) {
        host.innerHTML = `<p class="pv-empty">${esc(t().empty)}</p>`;
        return;
    }
    ...
    const svg = d3.select(host).append('svg')
        ...
        .attr('role', 'img')
        .attr('aria-label', t().ariaSeries(serie));
    ...
    // A change of provenance regime is the thing this panel exists to show, so it is drawn:
    // a dotted rule wherever the flag flips between one year and the next.
    if (!onlyObserved) {
        const breaks = [];
        for (let k = 1; k < points.length; k++) {
            const a = points[k - 1], b = points[k];
            if (a.flag !== b.flag && a.value != null && b.value != null) breaks.push(b.year);
        }
        g.append('g').selectAll('line.pv-break').data(breaks).join('line')
            .attr('class', 'pv-break')
            .attr('x1', yr => x(yr - 0.5)).attr('x2', yr => x(yr - 0.5))
            .attr('y1', 0).attr('y2', ih)
            .attr('stroke', '#8a6d3b').attr('stroke-width', 1)
...
function tipHTML(p, serie) {
    ...
    const chip = p.flag === FLAG_OBS ? L.observed : p.flag === FLAG_EST ? L.estimated : L.nodata;
    rows.push(`<div class="pv-chip" style="background:${mi.color}">${esc(chip)}</div>`);
    rows.push(`<dl class="pv-tip-dl">`);
    rows.push(`<dt>${esc(L.method)}</dt><dd>${esc(mi.label)}</dd>`);
    if (p.source) rows.push(`<dt>${esc(L.source)}</dt><dd>${esc(p.source)}</dd>`);
    if (p.confidence) {
        const c = (CONFIDENCE[LANG] || CONFIDENCE.es)[p.confidence] || p.confidence;
        rows.push(`<dt>${esc(L.confidence)}</dt><dd>${esc(c)}</dd>`);
    }
    if (p.donors && p.donors.length === 2) {
        rows.push(`<dt>${esc(L.between)}</dt><dd>${esc(fmtYear(p.donors[0]))} ${esc(L.and)} ${esc(fmtYear(p.donors[1]))}</dd>`);
    }
    rows.push(`</dl>`);
...
function ensureTip() {
    let el = document.getElementById('pv-tip');
    if (!el) {
...
const pct = (v) => new Intl.NumberFormat(locale(), { maximumFractionDigits: 1 }).format(v);

function coverageBar(c) {
    const L = t();
    const total = c.celdas || 1;
    const seg = (n, cls, label) => n
        ? `<span class="pv-bar-seg ${cls}" style="width:${(100 * n / total).toFixed(2)}%" title="${esc(label)}: ${n}"></span>`
        : '';
    return `
      <div class="pv-bar" role="img" aria-label="${esc(L.ariaBar(c))}">
        ${seg(c.observadas, 'obs', L.observed)}
        ${seg(c.estimadas, 'est', L.estimated)}
        ${seg(c.sin_dato, 'nod', L.nodata)}
      </div>
      <p class="pv-bar-cap">${L.barCap(c, pct(c.pct_observado))}</p>`;
}
...
function drawCoverage(host, cov) {
    ...
    const NAMES = { 0: L.nodata, 1: L.observed, 2: L.estimated };
    // 199 territories: a 7 px row made the labels overlap into an unreadable blur, so the
    // row is tall enough for its own label and the matrix scrolls in both directions.
    const cw = 9, chh = 11, labelW = 138;
    ...
    const svg = d3.select(wrap).append('svg')
        ...
        .attr('role', 'img')
        .attr('aria-label', L.ariaMatrix(cov.label));
    ...
    svg.append('g').selectAll('text').data(cov.territories).join('text')
        ...
        .text(d => (cov.labels?.[d] || d).slice(0, 22));
    ...
    svg.append('g').selectAll('rect').data(cells).join('rect')
        ...
        .on('pointerdown pointermove', (ev, d) => {
            const terr = cov.territories[d.r];
            tip.innerHTML =
                `<div class="pv-tip-h">${esc(cov.labels?.[terr] || terr)} · ${esc(cov.years[d.c])}</div>` +
                `<div class="pv-tip-v">${esc(NAMES[d.v])}</div>`;
            tip.style.display = 'block';
    ...
    note.innerHTML =
        `<span class="pv-sw" style="background:${COLORS[1]}"></span>${esc(L.observed.toLowerCase())} ` +
        `<span class="pv-sw" style="background:${COLORS[2]}"></span>${esc(L.estimated.toLowerCase())} ` +
        `<span class="pv-sw" style="background:${COLORS[0]}"></span>${esc(L.nodata.toLowerCase())}` +
        (cov.year_step > 1 ? L.everyN(cov.year_step) : '') +
...
const ProvenancePanel = {
    ...
    /** Set the language of every string this component draws. Call before rendering. */
    setLanguage(lang) { LANG = (lang === 'en' ? 'en' : 'es'); },

    /** Render one series panel into `host`, with its coverage bar, legend, toggle and download. */
    renderSeries(host, serie, opts = {}) {
        const L = t();
        const territory = opts.territoryLabel || serie.territory;
        const shown = { ...serie, territory, unit: opts.unit || serie.unit };
        host.innerHTML = `
          <div class="pv-panel">
            <header class="pv-head">
              <h3>${esc(territory)}</h3>
              <p class="pv-sub">${esc(opts.label || serie.label)}${shown.unit ? ` · ${esc(shown.unit)}` : ''} ·
            ...
            <div class="pv-actions">
              <label class="pv-toggle">
                <input type="checkbox" ${opts.onlyObserved ? 'checked' : ''}> ${esc(L.onlyObserved)}
              </label>
              <button type="button" class="pv-dl">${esc(L.download)}</button>
            </div>
          </div>`;

        const chart = host.querySelector('.pv-chart');
        let onlyObserved = !!opts.onlyObserved;
        drawSeries(chart, shown, { onlyObserved });
        ...
        host.querySelector('.pv-dl').addEventListener('click', () => {
            download(`${serie.id}.csv`, toCSV(shown));
        });
    },
